...
let atPlanet (planet: Planet) (state: State) =
    i18nWithParameters
        ...
        [ string planet.PopulationCount
          ...
          if state.DamageDetected
             && state.KSRLevel > (planet.KSRLevel - state.Offset) then
              "a threat"
          else
              "no threat"
          if state.DamageDetected then "damage" else "no damage"
          if state.KSRLevel > (planet.KSRLevel - state.Offset)
          then "superior"
          else "inferior"
          if state.DamageDetected
             && state.KSRLevel > (planet.KSRLevel - state.Offset) then
              (i18nNoParameters "planet.action.selfdestruct")
          else
              (i18nNoParameters "planet.action.eradicate") ]
